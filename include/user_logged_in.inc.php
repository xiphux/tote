...
function user_logged_in()
{
	...
	if (!$loggedinuser) {
		...
		$userstmt = $mysqldb->prepare("SELECT id, username, email, first_name, last_name, role, reminder, reminder_time, result_notification, timezone, style, (CASE WHEN (first_name IS NOT NULL AND last_name IS NOT NULL) THEN CONCAT(CONCAT(first_name,' '),last_name) WHEN first_name IS NOT NULL THEN first_name ELSE username END) AS display_name FROM " . TOTE_TABLE_USERS . " WHERE username=?");
