...
function get_pool_record($poolid)
{
	...
	$recorddetailstmt = $mysqldb->prepare("SELECT pool_records.user_id, pool_records.week, pool_records.team_id AS pick_team_id, pick_teams.id AS pick_team_id, pick_teams.abbreviation AS pick_team_abbr, pool_records.game_id AS game_id, home_teams.abbreviation AS home_team_abbr, away_teams.abbreviation AS away_team_abbr, games.home_score AS home_score, games.away_score AS away_score, win, loss, tie, ABS(spread) AS spread FROM " . TOTE_TABLE_POOL_RECORDS . " AS pool_records LEFT JOIN " . TOTE_TABLE_TEAMS . " AS pick_teams ON pool_records.team_id=pick_teams.id LEFT JOIN " . TOTE_TABLE_GAMES . " AS games ON pool_records.game_id=games.id LEFT JOIN " . TOTE_TABLE_TEAMS . " AS home_teams ON games.home_team_id=home_teams.id LEFT JOIN " . TOTE_TABLE_TEAMS . " AS away_teams ON games.away_team_id=away_teams.id WHERE pool_records.pool_id=? ORDER BY pool_records.week");
