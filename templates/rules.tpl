...
<li>A pick for a game must be submitted before the start of that game.  The website will not allow you to pick a game that has already started.</li>
...
{if $email}
<li>In the event of a technical difficulty (clicked the wrong button, encountered an error, etc), send an email to <a href="mailto:{$email}">{$email}</a> as soon as possible.  Administrator modification of bets due to technical issues will be allowed within 15 minutes of bet placement.</li>
{/if}
